...
<!-- Adding blank lines between code snippets breaks numbered lists. -->
...
1. Install the stable and nightly rust toolchains:
   ...
   ```
1. Install the [rustup target][rustup-target] for your Linux target platform:
   ```bash
   ARCH=x86_64
...
1. Install bpf-linker. Change the version number in `LLVM_SYS_<version>_PREFIX`
   to correspond to the major version of the [llvm-sys][llvm-sys] crate:
   ```bash
...
[rustup-target]: https://doc.rust-lang.org/nightly/rustc/platform-support.html#tier-1-with-host-tools
