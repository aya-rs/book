...
/// This crate has a runtime dependency on artifacts produced by the `xdp-drop-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "xdp-drop-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `xdp-drop-ebpf`, so cargo will automatically rebuild us
        // if `xdp-drop-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `xdp-drop-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("xdp-drop-ebpf");
