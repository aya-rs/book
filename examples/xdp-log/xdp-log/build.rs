...
/// This crate has a runtime dependency on artifacts produced by the `xdp-log-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "xdp-log-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `xdp-log-ebpf`, so cargo will automatically rebuild us
        // if `xdp-log-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `xdp-log-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("xdp-log-ebpf");
