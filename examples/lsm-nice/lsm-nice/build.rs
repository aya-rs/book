...
/// This crate has a runtime dependency on artifacts produced by the `lsm-nice-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "lsm-nice-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `lsm-nice-ebpf`, so cargo will automatically rebuild us
        // if `lsm-nice-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `lsm-nice-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("lsm-nice-ebpf");
