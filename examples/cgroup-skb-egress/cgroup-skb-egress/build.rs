...
use cargo_metadata::{
    Artifact, CompilerMessage, Message, Metadata, MetadataCommand, Package,
    Target,
};

/// This crate has a runtime dependency on artifacts produced by the `cgroup-skb-egress-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "cgroup-skb-egress-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `cgroup-skb-egress-ebpf`, so cargo will automatically rebuild us
        // if `cgroup-skb-egress-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `cgroup-skb-egress-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("cgroup-skb-egress-ebpf");
